...
class GetIntCentriodDP():
    ...
    def GetCenterS3_6c(self,features, pseudo_labels, centroidsLast=None, device=None):
        ...
        return outcentroids
def getCurrentPro(feature,pseudo_labels,confidence,out,device,getMask=False):
    ...
    if pseudo_labels is None:
        softmax_outputUP = F.softmax(out.detach(), dim=1)
        # print('softmax_output',softmax_outputUP.shape)
        pseudo_labelsUP = torch.argmax(softmax_outputUP, dim=1)
        pseudo_labels_probUP = torch.max(softmax_outputUP, dim=1)[0]  # 形状为 [B, H, W]
        pseudo_labelsUP[pseudo_labels_probUP < confidence] = 250
        maskA = torch.ones_like(pseudo_labelsUP,requires_grad=False).to(device)
        maskA[pseudo_labelsUP==250]=0
        # print('maskA',confidence,pseudo_labels_probUP.mean(),maskA.sum())
        # maskB = torch.ones_like(pseudo_labelsUP).to(device)
        # maskB[pseudo_labels_probUP < confidence] = 0
        # pseudo_labels=F.interpolate(pseudo_labelsUP.unsqueeze(1).float(), size=(32, 32),mode='nearest')
        pseudo_labels=pseudo_labelsUP
        # maskA=F.interpolate(maskA.unsqueeze(1).float(), size=(32, 32),mode='nearest')
        if getMask:
            return maskA
        # maskB=F.interpolate(maskB.unsqueeze(1).float(), size=(32, 32),mode='nearest')

        # print('maskA',confidence,pseudo_labels_probUP.mean(),maskA.sum(),maskA.shape)
        # print('maskB',confidence,pseudo_labels_probUP.mean(),maskB.sum(),maskB.shape)
        ...
        with torch.no_grad():
            for i in tbar:
                ...
                if STFlag:
                    ...
                    T_pred, _, TFeat = model.forward(T_full_img, DomainLabel=1, ProtoInput=None,getPFlag=2)
            ...
            for class_idx in range(6):
                ...
                if class_features_list[class_idx]:
                    ...
                    if self.n_cluster[class_idx] < max(self.n_cluster):
                        centroids = centroids.repeat(max(self.n_cluster)//self.n_cluster[class_idx], 0)
                    centroids = self.scaler.inverse_transform(centroids)
                    expand_Center = np.expand_dims(centroids, axis=0)
        ...
        with torch.no_grad():
            ...
            for class_idx in range(6):
                ...
                if class_features_list[class_idx]:
                    ...
                    # features_matrix = pca.fit_transform(features_matrix)
                    if self.n_cluster[class_idx] < 3:
                        X_normalized = features_matrix / np.linalg.norm(features_matrix, axis=1, keepdims=True)
                        centroids, _ = kmeans2(X_normalized, self.n_cluster[class_idx], minit='points')
                        centroids, labels = kmeans2(X_normalized, centroids, minit='matrix', dist=self.cosine_distance)
                        print('distance_matrix',X_normalized.shape,features_matrix,centroids.shape)

                        # similarity_matrix = cosine_similarity(features_matrix)
                        # distance_matrix = 1 - similarity_matrix
                        #
                        # kmeans = KMeans(n_clusters=self.n_cluster[class_idx], random_state=0)
...
class GetIntCentriodDPSTnoce():
    ...
    def getClusterKmean(self,model,iterFlag=False,STFlag=False,confidence_threshold=0.9,inputpro=None):
        self.model = model
        self.opt.phase = 'val'
        # Load Data
        S_dataload = self.dataloaderS.load_data()
        ...
        with torch.no_grad():
            for i in tbar:
                ...
                if STFlag:
                    ...
                    T_pred, _, TFeat = model.forward(T_full_img, DomainLabel=1, ProtoInput=inputpro,getPFlag=False)
                    FeatSim = TFeat['cat']
                    print('FeatSim',FeatSim.shape)
                    SSoft = F.softmax(FeatSim, dim=1)
                    SSoft = SSoft.view(SSoft.size(0), SSoft.size(1) // max(self.n_cluster), max(self.n_cluster),
                                       SSoft.size(2),
                                       SSoft.size(3))
                    pseudo_labels_prob = SSoft.max(dim=2)[0]
                    pseudo_labels = torch.argmax(pseudo_labels_prob.detach(), dim=1)
    ...
    def main(self,model,inputProto=None,epoch=1,dataloader=None,currentOpt=None,classifierFlag=False):
        ...
        with torch.no_grad():
            for i in tbar:
                ...
                # fig2, axs2 = plt.subplots(currentOpt.batch_size, 2 + 6, figsize=(11, 3 * currentOpt.batch_size))  # b行2列
                ...
                distancesSoftmaxMax, _ = torch.max(distancesSoftmax, dim=1)
                print('similarity_reshaped',similarity_reshaped.shape,distancesSoftmax.shape)
                np.save(self.savepath + 'WightPic/savedata/similarity_reshaped%d_%d.npy' % (epoch, i), similarity_reshaped.detach().cpu().numpy())
                np.save(self.savepath + 'WightPic/savedata/distancesSoftmax%d_%d.npy' % (epoch, i), distancesSoftmax.detach().cpu().numpy())
                np.save(self.savepath + 'WightPic/savedata/Probability%d_%d.npy' % (epoch, i), Probability.detach().cpu().numpy())
                # np.save(self.savepath + 'WightPic/savepic/labelT_%d_%d.npy' % (epoch, i), labelT_.detach().cpu().numpy())
                # np.save(self.savepath + 'WightPic/savepic/data_img_T%d_%d.npy' % (epoch, i), data_img_T.detach().cpu().numpy())
                ...
                if self.CatFlag:
                    ...
                    if classifierFlag:
                        ...
                        distancesSoftmax = F.softmax(FeatTTC['cat'], dim=1)
                        np.save(self.savepath + 'WightPic/savedata/distancesSoftmax2%d_%d.npy' % (epoch, i),
                                distancesSoftmax.detach().cpu().numpy())
        ...
        with torch.no_grad():
            for i in tbar:
                ...
                for ii in range(currentOpt.batch_size):
                    ...
                    cbar2 = fig1.colorbar(img2, ax=axs1[ii * 3, 1], extend='both', fraction=0.046, pad=0.04)
                    cbar2.set_ticks([0, 0.25, 0.5, 0.75, 1])
                    cbar2.ax.tick_params(labelsize=6)  # 设置刻度标签的字体大小为1
                    # cbar2.set_label('Probability')
                    for jj in range(2, 8):
                        # print(FeatTT['Weight'][-3].shape,'aaaaaaa')
                        img3 = axs1[ii * 3, jj].imshow(distancesSoftmax[ii, jj - 2].detach().cpu().numpy(),
