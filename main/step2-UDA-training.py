...
from model.deeplabv3plusPrototypeShow import DeepLabV3PlusSimGlobalLinearKL3neibor,SemanticSegmentationVarianceLoss,\
    SemanticSegmentationCosineLoss,neighborhood_relation_loss,compute_gradient_map_weight2,compute_gradient_map_weightP
...
def normalize2(logit):
    # mean = logit.mean(dim=-1, keepdims=True)
    # stdv = logit.std(dim=-1, keepdims=True)
    mean = logit.mean(dim=(2, 3), keepdim=True)
    stdv = logit.std(dim=(2, 3), keepdim=True)
    return (logit - mean) / (1e-7 + stdv)
def kd_loss2(logits_student_in, logits_teacher_in, temperature, logit_stand):
    logits_student = normalize(logits_student_in) if logit_stand else logits_student_in
    logits_teacher = normalize(logits_teacher_in) if logit_stand else logits_teacher_in
    log_pred_student = F.log_softmax(logits_student / temperature, dim=1)
    pred_teacher = F.softmax(logits_teacher / temperature, dim=1)
...
if __name__ == '__main__':
    ...
    name = opt.dset + '-z-EX/' + opt.model_type + '-step2-Cluster'
    opt.LChannel = False
    ...
    if opt.dset == 'mmlab':
        ...
        # cfg.TRAINLOG.DATA_NAMES = ['vailhingen6', 'potsdamRGB6']#potsdamRGB6Full,potsdamRGB6Full
    ...
    if opt.load_pretrain:
        ...
        # saveroot = '/datab/frb-project/AttenDA2/zmain/log/mmlab-z-EX/deeplabv3-step1/20240708-17_42_po-va-Neiborsource-2GAN-ST-IR/'
        # save_path = saveroot + '/savemodel/_25_acc-0.9315_miou-0.8476_mf1-0.9165.pth'
        # saveroot = '/datab/frb-project/AttenDA2/zmain/log/mmlab-z-EX/deeplabv3-step1/20240708-17_42_po-va-Neiborsource-2GAN-ST-IR/'
        # save_path = saveroot + '/savemodel/_25_acc-0.9315_miou-0.8476_mf1-0.9165.pth'
        # saveroot = '/datab/frb-project/AttenDA2/zmain/log/mmlab-z-EX/deeplabv3-step1/20240710-11_38_va-po-Neiborsource-2GAN/'
        # save_path = saveroot + '/savemodel/_31_acc-0.9238_miou-0.8037_mf1-0.8832.pth'
        saveroot = '/datab/frb-project/AttenDA2/zmain/log/mmlab-z-EX/deeplabv3-step1/20240710-11_37_va-po-Neiborsource-2GAN-IR/'
        save_path = saveroot + '/savemodel/_27_acc-0.9337_miou-0.8234_mf1-0.8964.pth'
    else:
        saveroot = None
        save_path = None
    ...
    time_now = time.strftime(
        ...
        '-noNeibor-noCompare-noCycleGAN-N=1-222244-IR',
        time.localtime())
    ...
    if opt.optimizer == 'sgd':
        ...
        # optimizer = torch.optim.SGD(netStudent.parameters(), lr=opt.lr*5,
        #                             momentum=0.9,
        #                             weight_decay=5e-4)
    ...
    GetIntCentriod_sourceST=GetIntCentriodDPST(opt=opt,dataloaderS=train_loader,dataloaderT=t_loader,
                                           device=DEVICE,Osize=outFeatSize,n_cluster=[2,2,2,2,4,4])
    ...
    # figure_target_metrics = Vailidate_Target.figure_metrics
    Vailidate_TargetSim2 = VailidateSim(opt=opt, device=DEVICE,Osize=outFeatSize,n_cluster=n_cluster,CatFlag=False)
    figure_TargetSim_metrics2 = Vailidate_TargetSim2.figure_metrics
    ...
    for epoch in range(10, opt.num_epochs + opt.num_decay_epochs + 1):
        #
        if epoch < 5:
            PrototypeScatST = GetIntCentriod_sourceST.getClusterKmean(model=netStudent, iterFlag=ttest,
                                                                      STFlag=False, confidence_threshold=None)
            ...
            if epoch<20:
                ...
                thr = 1.0 - (20) * 0.01
            PrototypeScatST = GetIntCentriod_sourceST.getClusterKmean(model=netStudent, iterFlag=ttest, STFlag=True,
        ...
        for i in tbar:
            ...
            seg_predS, PS, FeatS = netStudent.forward(images, DomainLabel=0, ProtoInput=PrototypeScatST)
            seg_predT, PT, FeatT = netStudent.forward(target_image, DomainLabel=0, ProtoInput=PrototypeScatST)
            # CurrentProT = getCurrentPro(feature=FeatT['asspF'], pseudo_labels=None, confidence=thr, device=DEVICE)
            # CurrentProS = getCurrentPro(feature=FeatS['asspF'], pseudo_labels=labelsP, confidence=None, device=DEVICE)
            # zero_mask_T = (CurrentProT == 0).all(dim=2, keepdim=True)
            # zero_mask_S = (CurrentProS == 0).all(dim=2, keepdim=True)
            # CurrentProS[zero_mask_T.expand_as(CurrentProS)] = 0
            # CurrentProT[zero_mask_S.expand_as(CurrentProT)] = 0
            #
            # CurrentProTsf = F.softmax(CurrentProT, dim=1)
            # CurrentProSlog = F.log_softmax(CurrentProS, dim=1)
            # loss_consist = F.kl_div(CurrentProSlog, CurrentProTsf, reduction='batchmean')

            # loss_consist = kd_loss(CurrentProS, CurrentProT, 2, True)
            # probabilities = F.softmax(seg_predT['out'].detach(), dim=1)
            # max_prob, pseudo_labels = torch.max(probabilities, dim=1)
            # mask = ((max_prob > thr).int()).unsqueeze(1)
            # # print('mask',mask.shape,mask.max(),mask.min())
            #
            # # neiborLossS = compute_gradient_map_weightP(similarity_map=FeatS['catmax'],
            # #                                            features=FeatS['asspF'], Proto=PrototypeScatST,
            # #                                            N_win_list=[2, 2, 2, 2, 1, 0], DEVICE=DEVICE)
            # # neiborLossT = compute_gradient_map_weightP(similarity_map=FeatT['catmax'],
            # #                                            features=FeatT['asspF'], Proto=PrototypeScatST,
            # #                                            N_win_list=[2, 2, 2, 2, 1, 0], DEVICE=DEVICE, mask=mask)
            # # neiborLoss = neiborLossS + neiborLossT
            # neiborLossS=compute_gradient_map_weight2(similarity_map=FeatS['catmax'],features=FeatS['asspF'],N_win_list=[2,2,2,2,1,0],DEVICE=DEVICE)
            # neiborLossT=compute_gradient_map_weight2(similarity_map=FeatT['catmax'],features=FeatT['asspF'],N_win_list=[2,2,2,2,1,0],DEVICE=DEVICE)
            #
            # neiborLoss = neiborLossS + 0.5*neiborLossT
            ...
            #     teacher_out, _, teacherFeat = netStudent.forward(target_image, DomainLabel=0,
            ...
            # loss_consist2 = kd_loss2(student_Feat['asspF'], teacher_out_affine,2,True)
            #
            # loss_consist2 = loss_consist2 * maskIn
            # loss_consist2 = loss_consist2.mean()
            # loss_consist2.backward()
        ...
        # Set PrototypeScat
        opt.phase = 'val'
        figure_val_metrics = Vailidate_Val.main(model=netStudent, figure_metrics=figure_val_metrics, epoch=epoch,
                                                cfg=cfg,
                                                iterFlag=ttest, dataloader=train_loader, currentOpt=opt,
                                                ProtoInput=PrototypeScatST)
        ###############    Val Similarity  #####################val
        opt.phase = 'val'
        figure_val_Sim_metrics = Vailidate_ValSim.main(model=netStudent, figure_metrics=figure_val_Sim_metrics,
                                                       epoch=epoch, cfg=cfg,
                                                       iterFlag=ttest, dataloader=train_loader, currentOpt=opt,
                                                       ProtoInput=PrototypeScatST)
        #####################################Global Proto####################3
        opt.phase = 'val'
        ProtoDist_Global_ProtoVal.main(model=netStudent, inputProto=PrototypeScatST, epoch=epoch, cfg=cfg,
        ...
        if epoch % 1 == 0:
            ...
            for kk in range(len(cfg.TRAINLOG.DATA_NAMES)):
                ...
                opt.phase = 'target'
                # Set PrototypeScat
                figure_TargetSim_metrics2 = Vailidate_TargetSim2.main(model=netStudent,
                                                                      figure_metrics=figure_TargetSim_metrics2,
                                                                      epoch=epoch, cfg=cfg,
                                                                      iterFlag=False, dataloader=t_loader,
                                                                      currentOpt=opt,
                                                                      ProtoInput=PrototypeScatST)  # 00Notice whether input Global prptotype
            ...
            for kk in range(len(cfg.TRAINLOG.DATA_NAMES)):
                ...
                t_loader = t_loaderTestDict[cfg.TRAINLOG.DATA_NAMES[kk]]
                opt.phase = 'target'
                SimilarityVisualizaion_Target.main(netStudent, inputProto=PrototypeScatT, epoch=epoch,
                                                   dataloader=t_loader, currentOpt=opt)
