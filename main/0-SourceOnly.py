...
if __name__ == '__main__':
    ...
    name = opt.dset + '-z-EX/' + opt.model_type + '-SourceOnly'
    opt.LChannel = False
    ...
    if opt.dset == 'mmlab':
        ...
        cfg.TRAINLOG.DATA_NAMES = ['va-poIRRGswin', 'potsdamIRRG6']
    ...
    #
    # flops = FlopCountAnalysis(netStudent, input_tensor)
    # print(f"FLOPs: {flops.total()}")

    summary(netStudent, input_size=input_size, col_names=["input_size", "output_size", "num_params", "mult_adds"])
    ...
    for epoch in range(start_epoch, opt.num_epochs + opt.num_decay_epochs + 1):
        ...
        if saveLast:
            if epoch == opt.num_epochs:
                ...
                tool.save_ckptGANCenter(network=[netStudent,None,None],Center=[None,None,None], optimizer=optimizer, save_str=save_str)
        else:
            save_str = './log/%s/%s/savemodel/_%d_acc-%.4f_miou-%.4f_mf1-%.4f.pth' \
                       % (name, time_now, epoch + 1, Vailidate_Val.val_scores['acc'], Vailidate_Val.val_scores['mIoU'], Vailidate_Val.val_scores['mf1'])
            ...
            tool.save_ckptGANCenter(network=[netStudent, None, None], Center=[None,None,None], optimizer=optimizer,
                                    save_str=save_str)
